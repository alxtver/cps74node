...
function loadPage(page, pages) {
  ...
  postData("/pcPa/pagination", data).then((data) => {
...
function getPage() {
  getData("/pcPa/getPage").then((page) => {
...
function load_pc(id) {
  ...
  postData("/pcPa/pc_edit", data).then((data) => {
...
function CreateTableEditPC(data, color) {
  ...
  button_back.setAttribute(
    "onclick",
    "location.href='/pcPa?part=" +
      data.part +
      "&serial_number=" +
      data.serial_number +
      "'"
  ...
  if (data.duplicatePki) {
    ...
    if (oldNumberMachine) {
      if (oldNumberMachine !== pcSN) {
        document.querySelector(".popup-checkbox").checked = true;
        document.getElementById("oldNumber").innerHTML =
          "Серийник был привязан к машине с номером " + oldNumberMachine;
        const audio = {};
        audio["alert"] = new Audio();
        audio["alert"].src = "/sounds/alert.mp3";
        audio["alert"].play();
...
async function insertSystemCase(id, obj, unit, serial_number) {
  const data = await putData("/pcPa/insertSystemCase", {
    id: id,
    obj: obj,
    serial_number: serial_number,
  });
  flashAlert(data);
  UpdateCells(data.pc, data.oldNumberMachine);
}
...
function edit_serial_number_apkzi(id, obj, unit, serial_number) {
  ...
  postData("/pcPa/insert_serial_apkzi", data).then((data) => {
    flashAlert(data);
    const serialNumber = data.pc.serial_number;
    const oldNumberMachine =
      data.oldNumberMachine !== serialNumber ? data.oldNumberMachine : null;
  ...
  if (oldNumberMachine) {
    ...
    if (!document.querySelector(".popup-checkbox").checked) {
      let nextCell = document.querySelector(
        ".serial_number[data-data='" + next_id.join(";") + "']"
      );
      let nextCellText;
      if (nextCell) {
...
function delBtn() {
  ...
  postData("/pcPa/delete", data).then((data) => {
    let page = data.page;
...
function selectPages(pageCount) {
  ...
  postData("/pcPa/pageCount", data).then(() => {
    setPage(1);
...
function submitFormAddPc() {
  ...
  formAddPC.addEventListener("submit", (e) => {
    ...
    postData("/pcPa/add", data).then((res) => {
      if (res.message === "ok") {
        window.location =
          "/pcPa?part=" +
          data.part +
...
function submitPC() {
  ...
  postData("/pcPa/pc_update", data).then((res) => {
    if (res.message === "ok") {
      window.location =
        "/pcPa?part=" +
        data.part +
